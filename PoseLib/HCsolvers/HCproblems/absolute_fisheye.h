...
class AbsoluteFisheyeHCProblem : public HCProblemBase<8, 8, Image> {
    ...
    // btw not polynomials cuz Trigonometric functions involved
...
class AbsoluteFisheyeHCProblemPoseLie : public HCProblemBase2<7, 7, Image> {
    // TODO: this class should include functions for specific problems, 
    // especially for the constraints evaluation and its jacobian and simulator
    // camera model is SimpleFisheyeCameraModel
    ...
    public:
        ...
        AbsoluteFisheyeHCProblemPoseLie(const std::vector<Point2D> &points2D, const std::vector<Point3D> &points3D,
                                 const Image &_pose_initial)
            : HCProblemBase2<7, 7, Image>(), pose_initial(_pose_initial) {
        ...
        void compute_polys(const Image &sol, const std::vector<Point2D> &x_, poly_t &polys) { 
            // input is the solution vector
            
            const Eigen::Vector3d t = sol.pose.t;
            const double f = sol.camera.params[0];
            const Eigen::Matrix3d R = sol.pose.R();
            const Eigen::Vector3d r1 = R.row(0);
            const Eigen::Vector3d r2 = R.row(1);
            const Eigen::Vector3d r3 = R.row(2);
        ...
        void compute_jacobian(const Image &sol, const std::vector<Point2D> &x_, jacobian_t &jacobian) {
